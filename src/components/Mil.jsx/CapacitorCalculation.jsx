...
function CapacitorCalculation({ onCalculate }) {  // Capacitor types data
  ...
  const [dcVoltageApplied, setDcVoltageApplied] = useState(0.001);
  const [acVoltageApplied, setAcVoltageApplied] = useState(0.001);
  const [dcVoltageRated, setDcVoltageRated] = useState(0.01);
  ...
  const [circuitResistance, setCircuitResistance] = useState(0.02);
  const [effectiveResistance, setEffectiveResistance] = useState(0.001);
  const [voltageApplied, setVoltageApplied] = useState(0.001);
  const [shouldCalculateCR, setShouldCalculateCR] = useState(false);

  const [piSR, setPiSR] = useState(0.01);
  ...
  //   return selectedCapacitor.πtColumn === 1 ? closestTemp.col1 : closestTemp.col2;
  // };
  const calculatePiT = () => {
  ...
  const handleCalculate = () => {
    ...
    const πC = calculatePiC();
    const πV = calculatePiV();
    const πSR = calculatePiSRFromCR(circuitResistance / voltageApplied);
    const πQ = selectedQuality.πQ;
    const πE = selectedEnvironment.πE;
