...
const MicrocircuitsCalculation = ({ onCalculate }) => {
  ...
  const eccOptions = [
    ...
    {
      value: "redundant",
      label: "Two-Needs-One Redundant Cell Approach",
      factor: 0.68,
    },
  ];
  ...
  const handleChange = (selectedOption) => {
    ...
    console.log(
      `Selected ECC: ${selectedOption.label}, Factor: ${selectedOption.factor}`
    );
  };
  ...
  const handleA2FactorChange = (selectedOption) => {
    ...
    console.log(
      `Selected A₂ Factor: ${selectedOption.label}, Value: ${selectedOption.a2Value}, Max Cycles: ${selectedOption.maxCycles}`
    );
  };
  ...
  const getQualityLabel = (piQ) => {
    ...
    return (
      qualityOptions.find((q) => q.piQ === parseFloat(piQ))?.label || "Unknown"
    );
  };
  ...
  const calculateSawDeviceFailureRate = (component) => {
    ...
    const piE = getEnvironmentFactor(component?.environment?.value);
     const final = (baseRate * piQ * piE)?.toFixed(6) 
    return final;
    };
         onCalculate = (final) => {
    setFailureRate(final);
  ...
  const dieComplexityRates = [
    ...
    },
    {
      ...
      rates: [
        ...
        { size: "64K < B ≤ 256K", mos: 0.005 },
        { size: "256K < B ≤ 1M", mos: 0.01 },
      ],
    },
    {
  ...
  const a1Factors = [
    { cycles: "Up to 100", flotox: 0.0007, texturedPoly: 0.0097 },
    ...
    { cycles: "1K < C ≤ 3K", flotox: 0.02, texturedPoly: 0.061 },
    { cycles: "3K < C ≤ 7K", flotox: 0.049, texturedPoly: 0.14 },
    { cycles: "7K < C ≤ 15K", flotox: 0.1, texturedPoly: 0.3 },
    { cycles: "15K < C ≤ 20K", flotox: 0.14, texturedPoly: 0.3 },
    { cycles: "20K < C ≤ 30K", flotox: 0.2, texturedPoly: 0.3 },
    { cycles: "30K < C ≤ 100K", flotox: 0.68, texturedPoly: 0.3 },
    { cycles: "100K < C ≤ 200K", flotox: 1.3, texturedPoly: 0.3 },
    { cycles: "200K < C ≤ 400K", flotox: 2.7, texturedPoly: 0.3 },
    { cycles: "400K < C ≤ 500K", flotox: 3.4, texturedPoly: 0.3 },
  ];
  ...
  const QUALITY_FACTORS = [
    {
      label:
        "10 Temperature Cycles (-55°C to +125°C) with end point electrical tests",
      value: 0.1,
      screeningLevel: "High",
    },
    {
  ...
  const calculateComponentSum = (newQuantity, currentComponent) => {
    ...
    const baseLambda =
      currentComponent?.baseFailureRate || mainInitialRate || 0;
  ...
  const getManufacturingFactor = () => {
    const selected = manufacturingProcessTypes?.find(
      (p) => p.value === inputs.manufacturingProcess
    );
  ...
  const getPackageFactor = () => {
    ...
    return inputs.packageHermeticity === "Hermetic"
      ? selected.hermetic
      : selected.nonhermetic;
  };
  ...
  const getESDFailureRate = () => {
    const selected = esdLevels?.find(
      (e) => e.value === inputs.esdSusceptibility
    );
  ...
  const getTechnologyFactor = () => {
    const technologyMap = {
      ...
      MOS: {
        basePiT: 0.1,
        description:
          "CMOS and MOS-based technologies (CMOS, Digital MOS, VHSIC)",
      },
      BiCMOS: {
        basePiT: 3.205e-8,
    ...
    return (
      technologyMap[currentComponent.technology] || {
        basePiT: 1.0,
  ...
  const calculateVhsicFailureRate = () => {
    try {
      ...
      const totalFailureRate =
        dieContribution + packageContribution + eosContribution;
  ...
  const calculateGaAsFailureRate = () => {
    try {
      ...
      if (!currentComponent.complexity)
        throw new Error("Please select complexity range");
      ...
      if (!currentComponent.pinCount)
        throw new Error("Please enter number of pins");
      if (!currentComponent.yearsInProduction)
        throw new Error("Please select years in production");
      if (!currentComponent.quality)
        throw new Error("Please select quality class");
  ...
  const calculateBubbleMemoryFailureRate = () => {
    ...
    if (
      !currentComponent.bubbleChips ||
      !currentComponent.dissipativeElements ||
      !currentComponent.numberOfBits ||
      !currentComponent.packageType ||
      !currentComponent.environment ||
      !currentComponent.quality ||
      !currentComponent.temperature
    ) {
    ...
    try {
      ...
      const C11 = 0.00095 * Math.pow(N1, 0.4);
      ...
      const D = currentComponent.writeDutyCycle
        ? parseFloat(currentComponent.writeDutyCycle)
        : 0;
      const RW = currentComponent.readsPerWrite
        ? parseFloat(currentComponent.readsPerWrite)
        : 2154;
      ...
      const lambda1 =
        piQ * (NC * C11 * piT1 * piW + (NC * C21 + C2) * piE) * piD * piL;
  ...
  const calculateHybridFailureRate = (component) => {
    ...
    console.log("piE", piE);
    const piQ =
      currentComponent.quality?.value ||
      getQualityFactor("MIL_M_38510_ClassB") ||
      1;
    console.log("piQ", piQ);
    const piF = getCircuitFunctionFactor();
    const piL =
      calculateLearningFactor(currentComponent.yearsInProduction) || 1;
  ...
  const addComponent = () => {
    ...
    switch (currentComponent.type) {
      case "Microcircuits,Gate/Logic Arrays And Microprocessors":
        failureRate =
          calculateMicrocircuitsAndMicroprocessorsFailureRate(currentComponent);
        calculationParams = {
          ...
          λp: calculateMicrocircuitsAndMicroprocessorsFailureRate(
            currentComponent
          ),
        };
        break;
    ...
    <div className="reliability-calculator">
      ...
      <h2 className="text-center" style={{ fontSize: "2.0rem" }}>
        {currentComponent?.type
          ? currentComponent.type.replace(/,/g, " ").trim()
      ...
      <div className="component-form">
        <Row className="mb-2">
            ...
            <div className="form-group">
              ...
              <Select
                ...
                options={[
                  ...
                  {
                    value: "Microcircuits,Memories",
                    label: "Microcircuits,Memories",
                  },
                  {
                    value: "Microcircuits,Hybrids",
                    label: "Microcircuits,Hybrids",
                  },
                  {
                    value: "Microcircuits,Saw Devices",
                    label: "Microcircuits,Saw Devices",
                  },
                  {
                    value: "Microcircuits,VHSIC/VHSIC-LIKE AND VLSI CMOS",
                    label: "Microcircuits,VHSIC/VHSIC-LIKE AND VLSI CMOS",
                  },
                  {
                    value: "Microcircuit,GaAs MMIC and Digital Devices",
                    label: "Microcircuit,GaAs MMIC and Digital Devices",
                  },
                  {
                    value: "Microcircuit,Magnetic Bubble Memories",
                    label: "Microcircuit,Magnetic Bubble Memories",
                  },
                ]}
              />
            </div>
          ...
          {currentComponent.type ===
            "Microcircuits,VHSIC/VHSIC-LIKE AND VLSI CMOS" && (
            <>
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Part Type (λ<sub>BD</sub>):
                  </label>
                  <Select
                    styles={customStyles}
              ...
              <Col md={4}>
                <div className="form-group">
                  <label>
                    ESD Susceptibility (V<sub>TH</sub>) (λ<sub>EOS</sub>):
                  </label>
                  <Select
                    styles={customStyles}
                    value={esdLevels?.find(
                      (e) => e.value === inputs.esdSusceptibility
                    )}
                    onChange={(selectedOption) => {
                      setInputs((prev) => ({
              ...
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Number of Package Pins (λ<sub>BP</sub>):
                  </label>
                  <input
                    type="number"
                    min="0"
                  ...
                  <Select
                    ...
                    options={[
                      ...
                      },
                      {
                        ...
                        piE: 2.0,
                        description:
                          "Permanent ground installations with environmental controls",
                      },
                      {
                        value: "GM",
                        label: "Ground, Mobile (GM)",
                      ...
                      },
                      {
                        ...
                        piE: 0.5,
                        description:
                          "Spacecraft in flight (not launch/re-entry)",
                      },
                      {
                        value: "MF",
                        label: "Missile, Flight (MF)",
                  ...
                  <Select
                    ...
                    options={[
                      ...
                      },
                      {
                        ...
                        piQ: 0.25,
                        description:
                          "Procured in full accordance with MIL-I-38535, Appendix B (Class U).",
                      },
                      {
                        ...
                        piQ: 0.25,
                        description:
                          "Hybrids procured to Class S (Quality Level K) of MIL-H-38534.",
                      },
                      ...
                      },
                      {
                        ...
                        piQ: 1.0,
                        description:
                          "Hybrids procured to Class B (Quality Level H) of MIL-H-38534.",
                      },
                      ...
                      // Class B-1 Category (πQ = 2.0)
                      {
                        ...
                        piQ: 2.0,
                        description:
                          "Compliant with MIL-STD-883, paragraph 1.2.1 (non-hybrid).",
                      },
                    ]}
                  />
                </div>
                    ...
                    options={[
                      {
                        ...
                        description:
                          "Early production phase (highest learning factor)",
                      },
                      {
                        value: 0.5,
                        label: "0.5 years",
                      ...
                      },
                      {
                        ...
                        piL: 1.0,
                        description:
                          "Mature production (lowest learning factor)",
                      },
                    ]}
                  />
                </div>
                  ...
                  <Select
                    ...
                    options={[
                      ...
                      },
                      {
                        ...
                        piQ: 5.0,
                        description:
                          "Standard commercial grade (lowest reliability)",
                      },
                    ]}
                ...
                <div className="form-group">
                  ...
                  <Select
                    ...
                    options={[
                      ...
                      {
                        value: "Video",
                        label: "Video (10MHz < f < 1GHz)",
                        piF: 1.2,
                      },
                      {
                        value: "Microwave",
                        label: "Microwave (f > 1GHz)",
                        piF: 2.6,
                      },
                      {
                        value: "Linear",
                        label: "Linear (f < 10MHz)",
                        piF: 5.8,
                      },
                ...
                <>
                  ...
                  <div className="d-flex flex-column gap-2">
                    <div className="d-flex align-items-center">
                      <strong>
                        Predicted Failure Rate (λ<sub>p</sub>):
                      </strong>
                      <span className="ms-2">
                        {results?.failureRate?.toFixed(6)} failures/10
                        <sup>6</sup> hours
                      ...
                      <div className="card-body">
                        <MaterialTable
                          columns={[
                            ...
                            },
                            {
                              ...
                              field: "contribution",
                              render: (rowData) =>
                                rowData?.componentSum?.toFixed(6) || "0.000000",
                            },
                            {
                              title: (
                                <span>
                        ...
                        <div className="formula-section mt-4">
                          ...
                          <div className="alert alert-light">
                            <code className="fs-5">
                              λ<sub>p</sub> = Σ(N<sub>c</sub> × λ<sub>c</sub>) ×
                              (1 + 0.2 π<sub>E</sub>) × π<sub>F</sub> × π
                              <sub>Q</sub> × π<sub>L</sub>
                            </code>
                          </div>
                          ...
                          <ul>
                            <li>
                              ...
                              = Predicted failure rate (Failures/10<sup>6</sup>{" "}
                              Hours)
                            </li>

                            <li>
                              <strong>
                                Σ(N<sub>c</sub> × λ<sub>c</sub>)
                              </strong>{" "}
                            ...
                            <li>
                              ...
                              = Failure Rate of each particular component
                            </li>
                            <li>
                              <strong>
                                N<sub>c</sub>
                          ...
                          <div className="alert alert-info mt-3">
                            <strong>Note:</strong> Only microcircuits, discrete
                            semiconductors, and capacitors contribute
                            significantly to the failure rate. Other components
                            are assumed to have λ<sub>c</sub> = 0.
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <Col md={4}>
                <div className="form-group">
                  ...
                  <Select
                    ...
                    options={[
                      {
                        value: "MMIC Devices-Low Noise & Low Power",
                        label: "MMIC Devices-Low Noise & Low Power",
                      },
                      {
                        value: "MMIC Devices-Driver & High Power",
                        label: "MMIC Devices-Driver & High Power",
                      },
                      {
                        value: "MMIC Devices-Unknown",
                        label: "MMIC Devices-Unknown",
                      },
                      {
                        value: "Digital Devices- All Digital Applications",
                        label: "Digital Devices- All Digital Applications",
                      },
                    ]}
                  />
                </div>
              ...
              <div className="calculator-container">
                <Row className="mb-3">
                  <Col md={4}>
                    <div className="form-group">
                      <label>
                        Package Type for (π<sub>prT</sub>):
                      </label>
                      <Select
                        styles={customStyles}
                        value={packageTypes?.find(
                          (p) => p.value === inputs.packageType
                        )}
                        onChange={(selectedOption) => {
                          setInputs((prev) => ({
                            ...prev,
                  ...
                  <Col md={4}>
                    <div className="form-group">
                      <label>
                        Technology Type for (π<sub>T</sub>):
                      </label>
                      <Select
                        styles={customStyles}
                        name="technology"
                        ...
                        options={[
                          {
                            ...
                            description:
                              "Standard TTL, Advanced Schottky TTL, and Current Mode Logic",
                            Ea: 0.4,
                          },
                          {
                            value: "F,LTTL,STTL",
                            ...
                            description:
                              "Fast, Low-Power, and Schottky TTL variants",
                            Ea: 0.45,
                          },
                          {
                            value: "BiCMOS",
                          ...
                          },
                          {
                            ...
                            description:
                              "High-speed/radiation-hardened silicon logic",
                            Ea: 0.6,
                          },
                          {
                            value: "Digital MOS",
                          ...
                          },
                          {
                            ...
                            description:
                              "Linear analog circuits (op-amps, regulators)",
                            Ea: 0.65,
                          },
                          {
                            value: "Memories",
                          ...
                          },
                          {
                            ...
                            description:
                              "Gallium Arsenide microwave/RF components",
                            Ea: 1.5,
                          },
                          {
                            value: "GaAs Digital",
                  ...
                  <Col md={4}>
                    <div className="form-group">
                      <label>
                        Junction Temperature (°C) (π<sub>T</sub>):
                      </label>
                      <input
                  ...
                  <Col md={4}>
                    <div className="form-group">
                      <label>
                        Feature Size (X<sub>S</sub>) microns (π<sub>CD</sub>):
                      </label>
                      <input
                        type="number"
                        min="0.1"
                  ...
                  <Col md={4}>
                    <div className="form-group">
                      <label>
                        Die Area (A) in cm² for (π<sub>CD</sub>):
                      </label>
                      <input
                        type="number"
                        min="0.01"
                  ...
                  <Col md={4}>
                    <div className="form-group">
                      <label>
                        Package Hermeticity for (π<sub>prT</sub>):
                      </label>
                      <Select
                        styles={customStyles}
                        value={{
                          ...
                          label:
                            inputs.packageHermeticity === "Hermetic"
                              ? "Hermetic"
                              : "Nonhermetic",
                        }}
                        onChange={(selectedOption) => {
                  ...
                  <Col md={4}>
                    <div className="form-group">
                      <label>
                        Manufacturing Process (π<sub>MFG</sub>):
                      </label>
                      <Select
                        styles={customStyles}
                        value={{
                          ...
                          label:
                            inputs.manufacturingProcess === "QML"
                              ? "QML"
                              : inputs.manufacturingProcess === "QPL"
                              ? "QPL"
                              : inputs.manufacturingProcess === "Non-QML"
                              ? "Non-QML"
                              : "Non-QPL",
                        }}
                        onChange={(selectedOption) => {
                  ...
                  {result && (
                    <>
                      ...
                      <div className="d-flex align-items-center">
                        ...
                        <span className="ms-2">
                          {result?.value} failures/10<sup>6</sup> hours
                        </span>
                      </div>
                    </>
                  )}
                          ...
                          <div className="card">
                            <div className="card-body">
                                ...
                                <MaterialTable
                                  columns={[
                                    ...
                                    },
                                    {
                                      ...
                                      field: "πE",
                                      render: (rowData) =>
                                        rowData?.inputs.environment?.factor ||
                                        "-", // Display the stored factor
                                    },
                                    {
                                      title: (
                                        <span>
                              ...
                              >
                                ...
                                <Typography variant="body1" paragraph>
                                  λ<sub>p</sub> = (λ<sub>BD</sub> × π
                                  <sub>MFG</sub>× π<sub>T</sub>× π<sub>CD</sub>)
                                  + (λ<sub>BP</sub>× π<sub>E</sub>× π
                                  <sub>Q</sub> × π<sub>prT</sub>) + λ
                                  <sub>EOS</sub>
                                </Typography>
                                <Typography variant="body1" paragraph>
                                  Where:
                                ...
                                <ul>
                                  <li>
                                    λ<sub>p</sub> = Predicted failure rate
                                    (Failures/10<sup>6</sup> Hours)
                                  </li>
                                  <li>
                                    λ<sub>BD</sub> = Die base failure rate
                                  </li>
                                  <li>
                                    π<sub>MFG</sub> = Manufacturing process
                                    correction factor
                                  </li>
                                  <li>
                                    π<sub>prT</sub> = Package type correction
                                    factor
                                  </li>
                                  <li>
                                    π<sub>CD</sub> = Die complexity correction
                                    factor
                                  </li>
                                  <li>
                                    λ<sub>BP</sub> = Package base failure rate
                                  </li>
                                  <li>
                                    λ<sub>EOS</sub> = Electrical overstress
                                    failure rate
                                  </li>
                                </ul>
                              </div>
          ...
          {currentComponent.type === "Microcircuits,Memories" && (
            <>
              ...
              <Col md={4}>
                <div className="form-group">
                  <label style={{ display: "block", marginBottom: "0.5rem" }}>
                    {mode === "A1"
                      ? "Programming Cycles (C) A1 for λcyc"
                      : "A₁ Value"}
                  </label>
                  <input
                    className="form-control"
                    ...
                    onChange={(e) => {
                      ...
                      if (value !== null) {
                        updatedComponent[
                          mode === "A1" ? "a1Value" : "programmingCycles"
                        ] =
                          mode === "A1"
                  ...
                  {currentComponent.programmingCycles !== null &&
                    mode === "C" && (
                      <div style={{ fontSize: "0.875rem", color: "#6c757d" }}>
                        Calculated Cycles:{" "}
                        {Math.round(currentComponent.programmingCycles)}
                      </div>
                    )}
                </div>
              </Col>
              <Col md={4}>
                <div className="form-group">
                  <label>
                    A₂ Factor for λ<sub>cyc</sub> (Textured-Poly):
                  </label>
                  <Select
                    styles={customStyles}
                    name="a2Factor"
              ...
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Error Correction Code Options (π<sub>ECC</sub>) for (λ
                    <sub>cyc</sub>) :
                  </label>
                  <Select
                    styles={customStyles}
                ...
                <div className="form-group">
                  ...
                  <Select
                    ...
                    value={environmentOptions.find(
                      (opt) => opt.value === currentComponent.environment?.value
                    )}
                    onChange={(selectedOption) => {
                      const updatedComponent = {
                        ...currentComponent,
                ...
                <div>
                  ...
                  <div className="d-flex align-items-center">
                    ...
                    <span className="ms-2 fw-bold">
                      {currentComponent?.calculatedFailureRate} failures/10
                      <sup>6</sup> hours
                    </span>
                  </div>
                  <br />
                        ...
                        <Col>
                          <div className="card">
                                ...
                                columns={[
                                  ...
                                  },
                                  {
                                    ...
                                    render: () => (
                                      <div>
                                        <span>
                                          {currentComponent.piQ?.toFixed(4) ||
                                            "Not calculated"}
                                        </span>
                                      </div>
                                    ),
                                  },
                                  {
                                    ...
                                    render: () => (
                                      <div>
                                        <span>
                                          {currentComponent.piE?.toFixed(4) ||
                                            "Not calculated"}
                                        </span>
                                      </div>
                                    ),
                            ...
                            <div className="formula-section mt-4 p-3 bg-light rounded">
                              ...
                              <p className="mb-1">
                                λ<sub>p</sub> = Base Rate × π<sub>Q</sub> × π
                                <sub>E</sub>
                              </p>
                              <p className="mb-1">Where:</p>
                              <ul className="mb-0">
                                <li>
                                  Base Rate = {BASE_FAILURE_RATE} (standard for
                                  SAW devices)
                                </li>
                                <li>
                                  π<sub>Q</sub> = Quality factor (from selected
                                  quality grade)
                                </li>
                                <li>
                                  π<sub>E</sub> = Environment factor (from
          ...
          {currentComponent.type ===
            "Microcircuit,GaAs MMIC and Digital Devices" && (
            <>
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Device Type for (C<sub>1</sub>):
                  </label>
                  <Select
                    styles={customStyles}
                    name="type"
                    ...
                    options={
                      currentDevice?.type === "MMIC"
                        ? [
                            {
                              value: "1-100",
                              label: "1-100 elements (C₁ = 4.5)",
                            },
                            {
                              value: "101-1000",
                              label: "101-1000 elements (C₁ = 7.2)",
                            },
                          ]
                        : [
                            {
                              value: "1-1000",
                              label: "1-1000 elements (C₁ = 25)",
                            },
                            {
                              value: "1001-10000",
                              label: "1001-10000 elements (C₁ = 51)",
                            },
                          ]
                    }
                  />
          ...
          {currentComponent.type ===
            "Microcircuit,Magnetic Bubble Memories" && (
            <>
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Environment Factor (π<sub>E</sub>):
                  </label>
                  <Select
                    styles={customStyles}
                    onChange={(selectedOption) => {
                    ...
                    options={[
                      {
                        value: "GB",
                        label: "GB - Ground Benign (πE = 0.50)",
                        piE: 0.5,
                      },
                      {
                        value: "GF",
                        label: "GF - Ground Fixed (πE = 2.0)",
                        piE: 2.0,
                      },
                      {
                        value: "GM",
                        label: "GM - Ground Mobile (πE = 4.0)",
                        piE: 4.0,
                      },
                      {
                        value: "NS",
                        label: "NS - Naval Sheltered (πE = 4.0)",
                        piE: 4.0,
                      },
                      {
                        value: "NU",
                        label: "NU - Naval Unsheltered (πE = 6.0)",
                        piE: 6.0,
                      },
                      {
                        value: "AIC",
                        label: "AIC - Airborne Inhabited Cargo (πE = 4.0)",
                        piE: 4.0,
                      },
                      {
                        value: "AIF",
                        label: "AIF - Airborne Inhabited Fighter (πE = 5.0)",
                        piE: 5.0,
                      },
                      {
                        value: "AUC",
                        label: "AUC - Airborne Uninhabited Cargo (πE = 5.0)",
                        piE: 5.0,
                      },
                      {
                        value: "AUF",
                        label: "AUF - Airborne Uninhabited Fighter (πE = 8.0)",
                        piE: 8.0,
                      },
                      {
                        value: "ARW",
                        label: "ARW - Airborne Rotary Wing (πE = 8.0)",
                        piE: 8.0,
                      },
                      {
                        value: "SF",
                        label: "SF - Space Flight (πE = 0.50)",
                        piE: 0.5,
                      },
                      {
                        value: "MF",
                        label: "MF - Missile Flight (πE = 5.0)",
                        piE: 5.0,
                      },
                      {
                        value: "ML",
                        label: "ML - Missile Launch (πE = 12)",
                        piE: 12,
                      },
                      {
                        value: "CL",
                        label: "CL - Cannon Launch (πE = 220)",
                        piE: 220,
                      },
                    ]}
                  />
          ...
          <>
                ...
                <div className="form-group">
                  ...
                  <Select
                    ...
                    options={[
                      {
                        value: "bipolarData",
                        label: "Bipolar Digital and Linear Gate",
                      },
                      {
                        value: "mosData",
                        label: "MOS Digital and Linear Gate",
                      },
              ...
              {currentComponent.devices === "bipolarData" && (
                <>
                  <Col md={4}>
                    <div className="form-group">
                      <label>
                        Bipolar Devices for (C<sub>1</sub>):
                      </label>
              ...
              {currentComponent.devices === "microprocessorData" && (
                <>
                  <Col md={4}>
                    <div className="form-group">
                      <label>
                        Microprocessor for (C<sub>1</sub>):
                      </label>
                      <Select
                        styles={customStyles}
                        name="complexFailure"
                  ...
                  <Col md={4}>
                    {currentComponent.complexFailure && (
                      <div className="form-group">
                        <label>
                          Microprocessor-{currentComponent.complexFailure} for
                          (C<sub>1</sub>):
                        </label>
                        <Select
                          styles={customStyles}
            ...
            <Row className="mb-2">
              <Col md={4}>
                <div className="form-group">
                  ...
                  <Select
                    ...
                    options={[
                      {
                        value: "Hermetic_DIPs_SolderWeldSeal",
                        label: "Hermetic: DIPs w/Solder or Weld Seal, PGA, SMT",
                      },
                      {
                        value: "DIPs_GlassSeal",
                        label: "DIPs with Glass Seal",
                      },
                      {
                        value: "Flatpacks_AxialLeads",
                        label: "Flatpacks with Axial Leads",
                      },
                      { value: "Cans", label: "Cans" },
                      {
                        value: "Nonhermetic_DIPs_PGA_SMT",
                        label: "Nonhermetic: DIPs, PGA, SMT",
                      },
                    ]}
                  />
                </div>
              ...
              <Col md={4}>
                <div className="form-group">
                  <label>
                    No. of Functional Pins for (C<sub>2</sub>):
                  </label>
                  <input
                    className="form-group"
                    style={{
            ...
            <Row className="mb-2">
              ...
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Junction Temperature (°C) for (π<sub>T</sub>):
                  </label>
                  <input
                    className="form-group"
                    style={{
                  ...
                  <div className="card-body">
                          ...
                          data={[
                            {
                              ...
                              piE: getEnvironmentFactor(
                                currentComponent.environment
                              ),
                              piQ: getQualityFactor(currentComponent.quality),
                              piL: calculateLearningFactor(
                                currentComponent.yearsInProduction
                              ),
                              λp: calculateMicrocircuitsAndMicroprocessorsFailureRate(
                                currentComponent
                              ),
                    ...
                    <div className="formula-section">
                      {currentComponent.type ===
                        ...
                        <>
                          <p>
                            FR = (C<sub>1</sub> × π<sub>T</sub> + C<sub>2</sub>{" "}
                            × π<sub>E</sub>) × π<sub>Q</sub> × π<sub>L</sub>
                          </p>
                          <p>Where:</p>
                          <ul>
                            <li>
                              C<sub>1</sub> = Base failure rate (depends on gate
                              count and complexity)
                            </li>
                            <li>
                              C<sub>2</sub> = Package failure rate (from
                              MIL-HDBK-217 tables)
                            </li>
                            <li>
                              π<sub>T</sub> = Temperature factor (technology
                              dependent)
                            </li>
                            <li>
                              π<sub>E</sub> = Environment factor (GB, GF, GM,
                              etc.)
                            </li>
                            <li>
                              π<sub>Q</sub> = Quality factor (MIL-SPEC grade)
                            </li>
                            <li>
                              π<sub>L</sub> = Learning factor (years in
                              production)
                            </li>
                          </ul>
                        </>
          ...
          <>
            <Row>
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Memory Technology for (B<sub>1</sub>):
                  </label>
                  <Select
                    styles={customStyles}
                    name="memoryTech"
              ...
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Memory Size (Textured-Poly<sup>3</sup> )for B₂:
                  </label>
                  <Select
                    styles={customStyles}
                    name="memorySizeB2"
              ...
              <div className="card mt-3">
                <div className="card-body">
                  ...
                  <div className="mt-3">
                    ...
                    <p>
                      λ<sub>p</sub> = (C<sub>1</sub> × π<sub>T</sub> + C
                      <sub>2</sub> × π<sub>E</sub> + λ<sub>cyc</sub>) × π
                      <sub>Q</sub>× π<sub>L</sub>
                    </p>
                    ...
                    <ul>
                      <li>
                        C<sub>1</sub> = Die complexity failure rate (from memory
                        type and size)
                      </li>
                      <li>
                        π<sub>T</sub> = Temperature factor (based on junction
                        temperature)
                      </li>
                      <li>
                        C<sub>2</sub> = Package failure rate (from package type
                        and pin count)
                      </li>
                      <li>
                      ...
                      {inputs.memoryType.type.includes("EEPROM") && (
                        <li>
                          λ<sub>cyc</sub> = Read/write cycling induced failure
                          rate (for EEPROMs)
                        </li>
                      )}
                      <li>
                        π<sub>Q</sub> = Quality factor
                      </li>
                    </ul>
                  </div>
                </div>
          ...
          <>
            <Row className="mb-3">
                ...
                <div className="form-group">
                  ...
                  <Select
                    ...
                    options={[
                      {
                        value: "GB",
                        label: "Ground Benign (πE = 0.5)",
                        piE: 0.5,
                      },
                      {
                        value: "GF",
                        label: "Ground Fixed (πE = 2.0)",
                        piE: 2.0,
                      },
                      {
                        value: "GM",
                        label: "Ground Mobile (πE = 4.0)",
                        piE: 4.0,
                      },
                      {
                        value: "NS",
                        label: "Naval Sheltered (πE = 4.0)",
                        piE: 4.0,
                      },
                      {
                        value: "NU",
                        label: "Naval Unsheltered (πE = 6.0)",
                        piE: 6.0,
                      },
                      {
                        value: "AIC",
                        label: "Airborne Cargo (πE = 4.0)",
                        piE: 4.0,
                      },
                      {
                        value: "AIF",
                        label: "Airborne Fighter (πE = 5.0)",
                        piE: 5.0,
                      },
                      {
                        value: "AUC",
                        label: "Airborne Uninhabited Cargo (πE = 5.0)",
                        piE: 5.0,
                      },
                      {
                        value: "AUF",
                        label: "Airborne Uninhabited Fighter (πE = 8.0)",
                        piE: 8.0,
                      },
                      {
                        value: "ARW",
                        label: "Airborne Rotary Wing (πE = 8.0)",
                        piE: 8.0,
                      },
                      {
                        value: "SF",
                        label: "Space Flight (πE = 0.5)",
                        piE: 0.5,
                      },
                      {
                        value: "MF",
                        label: "Missile Flight (πE = 5.0)",
                        piE: 5.0,
                      },
                      {
                        value: "ML",
                        label: "Missile Launch (πE = 12)",
                        piE: 12,
                      },
                      {
                        value: "CL",
                        label: "Cannon Launch (πE = 220)",
                        piE: 220,
                      },
                    ]}
                  />
              ...
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Application Factor (π<sub>A</sub>):
                  </label>
                  <Select
                    styles={customStyles}
                    placeholder="Select"
                    ...
                    options={[
                      {
                        value: "LowNoiseLowPower",
                        label: "Low Noise & Low Power (πA = 1.0)",
                        piA: 1.0,
                      },
                      {
                        value: "DriverHighPower",
                        label: "Driver & High Power (πA = 3.0)",
                        piA: 3.0,
                      },
                      {
                        value: "Unknown",
                        label: "Unknown (πA = 3.0)",
                        piA: 3.0,
                      },
                      {
                        value: "Digital",
                        label: "Digital Applications (πA = 1.0)",
                        piA: 1.0,
                      },
                    ]}
                  />
                    ...
                    <div className="card">
                      <div className="card-body">
                        ...
                        >
                          ...
                          <Typography variant="body1" paragraph>
                            λ<sub>p</sub> = [C<sub>1</sub> × π<sub>T</sub> × π
                            <sub>A</sub> + C<sub>2</sub> × π<sub>E</sub>] × π
                            <sub>L</sub> × π<sub>Q</sub>
                          </Typography>
                          <Typography variant="body1" paragraph>
                            Where:
                          </Typography>
                          <ul>
                            ...
                            <li>
                              C<sub>1</sub> = Die complexity failure rate
                            </li>
                            <li>
                              π<sub>T</sub> = Temperature factor
                            </li>
                            <li>
                              π<sub>A</sub> = Application factor
                            </li>
                            <li>
                              C<sub>2</sub> = Package failure rate
                            </li>
                            <li>
                              π<sub>E</sub> = Environment factor
                            </li>
                            <li>
                              π<sub>L</sub> = Learning factor
                            </li>
                            <li>
                              π<sub>Q</sub> = Quality factor
                            </li>
          ...
          <>
            <Row className="mb-3">
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Number of Bubble Chips per Package (N<sub>C</sub>):
                  </label>
                  <input
                    className="form-control"
                    type="number"
              ...
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Dissipative Elements (N<sub>1</sub>):
                  </label>
                  <input
                    className="form-control"
                    type="number"
            ...
            <Row className="mb-3">
                ...
                <div className="form-group">
                  ...
                  <Select
                    ...
                    options={[
                      {
                        value: "Hermetic_DIPs_SolderWeldSeal",
                        label: "Hermetic: DIPs w/Solder or Weld Seal",
                        c2: 0.00092,
                      },
                      {
                        value: "DIPs_GlassSeal",
                      ...
                      {
                        value: "Nonhermetic_DIPs",
                        label: "Nonhermetic: DIPs",
                        c2: 0.0012,
                      },
                    ]}
                  />
              ...
              <Col md={4}>
                <div className="form-group">
                  ...
                  <input
                    ...
                    max="175"
                    value={
                      currentComponent.tcase
                        ? Number(currentComponent.tcase) + 10
                        : ""
                    }
                    readOnly
                  />
                  <small className="form-text text-muted">
                    T<sub>J</sub> = T<sub>CASE</sub> + 10°C (automatically
                    calculated)
                  </small>
                </div>
              </Col>
              ...
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Reads per Write (R/W) for (π<sub>W</sub>):
                  </label>
                  <input
                    className="form-control"
                    type="number"
              ...
              <Col md={4}>
                <div className="form-group">
                  <label>
                    Write Duty Cycle Factor (π<sub>W</sub>):
                  </label>
                  <input
                    className="form-control"
                    type="number"
            ...
            {result && showCalculations && (
              <>
                  ...
                  <MaterialTable
                    columns={[
                      ...
                      },
                      {
                        ...
                        field: "lambda1",
                        render: (rowData) =>
                          rowData?.lambda1?.toFixed(6) || "-",
                      },
                      {
                        title: "λ₂ (Memory)",
                        field: "lambda2",
                        render: (rowData) =>
                          rowData?.lambda2?.toFixed(6) || "-",
                      },
                      {
                        title: "λₚ (Total)",
                        field: "lambdaP",
                ...
                <div className="formula-section mt-3">
                  <h5>Calculation Formula</h5>
                  <p>
                    λ<sub>p</sub> = λ<sub>1</sub> + λ<sub>2</sub>
                  </p>
                  <p>Where:</p>
                  <p>
                    λ<sub>1</sub> = π<sub>Q</sub>[N<sub>C</sub>C<sub>11</sub>π
                    <sub>T1</sub>π<sub>W</sub> + (N<sub>C</sub>C<sub>21</sub> +
                    C<sub>2</sub>)π<sub>E</sub>]π<sub>D</sub>π<sub>L</sub>
                  </p>
                  <p>
                    λ<sub>2</sub> = π<sub>Q</sub>N<sub>C</sub>(C<sub>12</sub>π
                    <sub>T2</sub> + C<sub>22</sub>π<sub>E</sub>)π<sub>L</sub>
                  </p>
                </div>
              </>
            )}
