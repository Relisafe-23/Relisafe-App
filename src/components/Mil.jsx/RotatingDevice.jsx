...
const RotatingDevice = ({ onCalculate }) => {
  ...
  const [currentComponent, setCurrentComponent] = useState({
    ...
    circuitType: "",
    yearsInProduction: 1,
    quality: { value: 1, label: "MIL-M-38510 Class B" },
    ratioBInput: "",
    ratioWInput: "",
    designLife: 87600,
    ambientTemp: 30,
    isCycledTemp: false,
    tempCycles: [{ temp: 30, hours: 87600 }],
    frameTemp: 40,
    useAmbientAsFrameTemp: true,
    selectedSynchroType: 'synchro',
    selectedSize: '8 or smaller',
    selectedBrushCount: '≤ 2',
    selectedTimeMeter: timeMeterTypes[0],
    operatingTempRatio: 1.0,
    selectedElapsedEnvironment: elapsedEnvironmentFactors[0]
  });
  ...
  const calculateMotorFailureRate = () => {
    ...
    if (currentComponent.isCycledTemp) {
      ...
      const life = getBearingWindingLife(currentComponent.ambientTemp);
      αB = life.αB;
      αW = life.αW;
    }
  ...
  const calculateSynchroFailureRate = () => {
    const actualFrameTemp = currentComponent.useAmbientAsFrameTemp ? currentComponent.ambientTemp : currentComponent.frameTemp;

    // Find closest temperature in the table
    const exactMatch = synchroBaseRates.find(item => item.temp === actualFrameTemp);
    let λb;
    ...
    if (exactMatch) {
      ...
      const sorted = [...synchroBaseRates].sort((a, b) => a.temp - b.temp);
      let lower = sorted[0];
      let upper = sorted[sorted.length - 1];

      for (let i = 0; i < sorted.length - 1; i++) {
        if (sorted[i].temp <= actualFrameTemp && sorted[i + 1].temp >= actualFrameTemp) {
          lower = sorted[i];
          upper = sorted[i + 1];
          break;
        }
      ...
      const ratio = (actualFrameTemp - lower.temp) / (upper.temp - lower.temp);
      λb = lower.λb + ratio * (upper.λb - lower.λb);
    }

    const sizeFactor = sizeFactors.find(
      item => item.type === currentComponent.selectedSynchroType && item.size === currentComponent.selectedSize
    )?.factor || 1;
      ...
      <Row>
        ...
        {currentComponent.type === 'motor' && (
          <>
            ...
            <Col md={4}>
              <div className="form-group">
                <label>LC/α<sub>B</sub> Ratio:</label>
                <Select
                  styles={customStyles}
                  value={{
            ...
            <Col md={4}>
              <div className="form-group">
                <label>LC/α<sub>W</sub> Ratio:</label>
                <Select
                  styles={customStyles}
                  value={{
            ...
            {currentComponent.isCycledTemp && (
              <Col md={12}>
                <div className="form-group">
                  ...
                  <Button variant="secondary" onClick={addTempCycle}>
          ...
          <>
            ...
            <Col md={4}>
              <div className="form-check">
                ...
                />
                <label className="form-check-label" htmlFor="useAmbientAsFrameTemp">
                  Use Ambient as Frame Temperature
                </label>
              </div>
            </Col>
