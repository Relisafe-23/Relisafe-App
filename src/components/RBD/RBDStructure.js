...
/** Height a single branch needs based on how many blocks it has (all same size) */
// const branchHeight = (branch) => {
//   const cnt = (branch.blocks || []).length;
//   // with blocks: BLOCK_H. Empty: BLOCK_H. Always BLOCK_H per branch row.
//   return C.BRANCH_MIN_H;
// };

// Also update branchHeight function to account for nested sections
const branchHeight = (branch) => {
...
/**
 * Total height of a parallel section:
 * sum of each branch's individual height + spacing between them
 */
...
/**
 * Width a parallel section needs.
 * leftGrowth = how much the section has expanded leftward.
 * Right side expands by maxBlocks in any branch.
 */
...
/** y-center of a branch given the section's topY and the branches above it */
...
// In the renderParallelSection function, update the branchCenterY function
const branchCenterY = (branches, idx, secTopY) => {
  let y = secTopY;
  for (let i = 0; i < idx; i++) {
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '20px', overflowX: 'auto' }}>
      ...
      {kOfNModal.open && (
        <KOfNConfigModal
          isOpen
          onClose={() => { setParentItem(null); setKOfNModal({ open: false, blockId: null, initialData: null, mode: 'add', nodeIndex: null }); }}
